...
impl Model {
    ...
    pub async fn get_subscriber(&self, raw_email: &str) -> Result<Option<Subscriber>, sqlx::Error> {
        let email = clean_email(raw_email).expect("Invalid email address");
        let sql = r#"
        SELECT * FROM subscribers WHERE email = ?1
    "#;

        let user = sqlx::query_as::<_, Subscriber>(sql)
            .bind(email)
    ...
    pub async fn subscribe(&self, raw_email: &str) -> Result<usize, sqlx::Error> {
        self.add_subscriber(raw_email).await
    }
    ...
    pub async fn mark_email_as_failure(
        &self,
        raw_email: &str,
        item_guid: &str,
    ) -> Result<usize, sqlx::Error> {
